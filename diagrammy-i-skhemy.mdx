...
```mermaid
flowchart TD
    A["👤 Пользователь открывает Lumos"] --> B["📱 Вводит номер телефона<br/>+7 999 123-45-67"]
...
```mermaid
stateDiagram-v2
    [*] --> ClientWriting: Клиент пишет в Avito
    ClientWriting --> SystemSync: Система загружает<br/>каждые 15-30 мин
    SystemSync --> ManagerView: Появляется в ленте<br/>менеджера
    ManagerView --> Active: Статус: АКТИВНЫЙ 🟢<br/>Непрочитан<br/>Требует ответа
    Active --> ManagerReply: Менеджер видит<br/>и отвечает
    ManagerReply --> PlatformSync: Ответ отправляется<br/>на Avito/Cian/Domclick
    PlatformSync --> ClientReceive: Клиент видит<br/>ответ в приложении
    ClientReceive --> Inactive: Чат становится<br/>НЕАКТИВНЫМ ⚪<br/>(клиент не пишет)
    Inactive --> [*]
```
...
```mermaid
graph TB
    Lumos["🎯 LUMOS API<br/>Центральная система"]

    Avito["🔴 AVITO"]
    Cian["🔵 CIAN"]
    Domclick["🟢 DOMCLICK"]

    AvitoAPI["OAuth авторизация<br/>XML Feeds<br/>Avito API"]
    CianAPI["API Chats<br/>Cian API"]
    DomclickAPI["API Stats<br/>Domclick API"]

    AvitoData["📋 Объявления<br/>💬 Чаты<br/>💭 Сообщения"]
    CianData["💬 Чаты<br/>💭 Сообщения"]
    DomclickData["💬 Чаты<br/>💭 Сообщения"]
    ...
    style Lumos fill:#fff3e0,stroke:#ff9800,stroke-width:3px
