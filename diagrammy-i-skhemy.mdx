...
stateDiagram-v2
    ...
    ClientWriting --> SystemSync: Система загружает\nкаждые 15-30 мин
    SystemSync --> ManagerView: Появляется в ленте\nменеджера
    ManagerView --> Active: Статус: АКТИВНЫЙ\nНепрочитан\nТребует ответа
    Active --> ManagerReply: Менеджер видит\nи отвечает
    ManagerReply --> PlatformSync: Ответ отправляется\nна Avito / Cian / Domclick
    PlatformSync --> ClientReceive: Клиент видит\nответ в приложении
    ClientReceive --> Inactive: Чат становится\nНЕАКТИВНЫМ\n(клиент не пишет)
    Inactive --> [*]
```
